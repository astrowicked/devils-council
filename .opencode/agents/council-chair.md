...
- (removed — filesystem references not used in OpenCode) — especially `personas_run[]` (each entry has `name`,
  `trigger_reason`, `outcome`, and after Plan 05-01 stamping, `findings[]` with
  `id`, `target`, `claim`, `severity`, `category`).
- (removed — filesystem references not used in OpenCode) for each entry in `personas_run[]` whose outcome is
...
Write your draft to (removed — filesystem references not used in OpenCode). Do NOT write the final
(removed — filesystem references not used in OpenCode) — the conductor's `bin/dc-validate-synthesis.sh`
