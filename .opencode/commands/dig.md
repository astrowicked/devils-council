...
if ! printf '%s' "$PERSONA" | grep -qE '^[a-z][a-z-]*$'; then
  ...
  if [ -d "${CLAUDE_PLUGIN_ROOT:-$HOME/.cache/opencode/packages/devils-council-opencode@latest/node_modules/devils-council-opencode}/agents" ]; then
    (cd "${CLAUDE_PLUGIN_ROOT:-$HOME/.cache/opencode/packages/devils-council-opencode@latest/node_modules/devils-council-opencode}/agents" && ls -1 *.md 2>/dev/null | grep -v -iE '^README' | sed 's/\.md$//' | sed 's/^/  /') >&2
  fi
  exit 2
fi
