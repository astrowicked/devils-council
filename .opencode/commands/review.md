...
if [ -n "${CLAUDE_PLUGIN_ROOT:-}" ] && [ -d "$CLAUDE_PLUGIN_ROOT/bin" ]; then
  ...
  DC=$(find "$HOME/.cache/opencode" "$HOME/.claude/plugins" -path "*/devils-council*/bin/dc-prep.sh" 2>/dev/null | head -1)
  if [ -n "$DC" ]; then
...
The block above printed `DC_ROOT=<path>`. Use this as the plugin root for ALL subsequent `bin/` and `lib/` references in this command.

If `DC_ROOT=ERROR:not-found`, STOP. Tell the user: "Cannot locate devils-council bin/ directory. Ensure the plugin is installed: check `~/.cache/opencode/packages/` or `~/.claude/plugins/`."

For all subsequent shell-injection blocks, prefix script paths with the resolved `DC_ROOT` value (NOT `${DC_ROOT}`).
...
!`${DC_ROOT}/bin/dc-prep.sh $ARGUMENTS`

!`${DC_ROOT}/bin/dc-classify.sh "$(ls -t .council/*/INPUT.md 2>/dev/null | head -1)" "$(ls -t .council/*/MANIFEST.json 2>/dev/null | head -1)"`
...
If `HAS_CLASSIFIER` is `false`, invoke the classifier explicitly:
    ...
    ${DC_ROOT}/bin/dc-classify.sh "<RUN_DIR>/INPUT.md" "<RUN_DIR>/MANIFEST.json"
...
Use the Bash tool to execute:
    ...
    ${DC_ROOT}/bin/dc-budget-plan.sh <RUN_DIR> \
      ${ONLY:+--only="$ONLY"} \
...
2. Use the Bash tool to execute:
       ...
       ${DC_ROOT}/bin/dc-codex-delegate.sh P <RUN_DIR>
...
**Case B — draft file exists:** Use the **Bash tool** to execute:
    ...
    ${DC_ROOT}/bin/dc-validate-scorecard.sh P <RUN_DIR> core:always-on
...
SUPPRESSED_IDS line from stdout for the render stage below:
    ...
    ${DC_ROOT}/bin/dc-apply-responses.sh <RUN_DIR>
...
exists on disk, use the **Bash tool** to execute:
    ...
    ${DC_ROOT}/bin/dc-validate-synthesis.sh <RUN_DIR>
...
After all scorecards and SYNTHESIS.md are written, generate a single-page HTML report:

!`if command -v python3 >/dev/null 2>&1; then
  RENDER_SCRIPT="${DC_ROOT}/bin/dc-render-html.py"
  if [ -f "$RENDER_SCRIPT" ]; then
    python3 "$RENDER_SCRIPT" "<RUN_DIR>" 2>&1 || true
  fi
fi`

If the render succeeds, include the report path in the final output:
